"""Convert the RA hour XLSX sheets provided by MISO into a single feather file.
...
OUTPUT_PATH = INPUT_DIR / "miso_ra_hours.feather"

SEASONAL_NORTH = "Seasonal RA Hour_RA Hour Identifier (Central + North)"
SEASONAL_SOUTH = "Seasonal RA Hour_RA Hour Identifier (South)"
AAOC_NORTH = "Annual RA Hour (AAOC Hour)_RA Hour Identifier (Central + North)"
    ...
    out = pd.DataFrame(
        {
            ...
            "ra_north": raw[SEASONAL_NORTH].astype(int).astype(bool),
            "ra_south": raw[SEASONAL_SOUTH].astype(int).astype(bool),
        }
    )
    if AAOC_NORTH in raw.columns:
        out["aaoc_north"] = raw[AAOC_NORTH].astype(int).astype("boolean")
        out["aaoc_south"] = raw[AAOC_SOUTH].astype(int).astype("boolean")
