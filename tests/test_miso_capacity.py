...
def test_limit_pro_rata_scales_columns_equally():
    ...
    With a=4 and b=4 each hour (row-sum=8) and a limit of 4, every
    component is multiplied by 4/8=0.5, yielding 2.0 for both.
    """
    df = _make_hourly_df(
        "2024-01-01 00:00",
        2,
...
def test_limit_pro_rata_treats_each_hour_independently():
    ...
    Hour 0 (a=1, b=1, row-sum=2) is below the limit of 4 and is
    untouched.  Hour 1 (a=5, b=5, row-sum=10) is scaled by 4/10 so both
    columns become 2.0.
    """
    df = _make_hourly_df(
        "2024-01-01 00:00",
...
def test_limit_priority_order_list_reduces_last_first():
    ...
    absorbs all 2 MW of excess (row-sum=8, limit=6) before "a" is touched.
    """
    df = _make_hourly_df(
        "2024-01-01 00:00",
        2,
...
def test_limit_priority_order_list_cascades_when_last_zeroed():
    ...
    Excess=3 (row-sum=5, limit=2).  "b" can only provide 1, so the
    remaining 2 is absorbed by "a", reducing it from 4 to 2.
    """
    df = _make_hourly_df(
        "2024-01-01 00:00",
        2,
...
def test_limit_priority_order_dict_varies_by_season():
    ...
    (order ["b","a"]).  Each row has row-sum=8 and limit=6, so exactly
...
def test_class_ucap_and_isac_have_correct_keys():
    ...
    for component in mc.component_list:
        for season in ("summer", "fall", "winter", "spring"):
            assert (season, component) in mc.class_ucap
            assert (season, component) in mc.class_isac
...
def test_class_ucap_wind_summer_matches_csv_value():
    ...
    assert mc.class_ucap[("summer", "a")] == 2079.0
...
def test_class_isac_wind_winter_matches_csv_value():
    ...
    """
    mc = _availability_capacity()
    assert mc.class_isac[("winter", "a")] == 10859.0
...
def test_class_ucap_isac_conversion_is_ratio():
    ...
    for key in mc.class_ucap_isac_conversion:
        ucap = mc.class_ucap[key]
        isac = mc.class_isac[key]
...
def test_sac_equals_isac_times_conversion():
    ...
    mc = _availability_capacity()
    for key, sac_value in mc.sac.items():
        expected = mc.isac[key] * mc.class_ucap_isac_conversion[key]
...
def test_revenue_per_season_units_are_dollars():
    ...
    if fall_revenue is not None:
        zrc = mc.zrc.get(("fall", "a"), float("nan"))
